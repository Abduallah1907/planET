...
export class TouristController {
  ...
  public async bookItinerary(req: any, res: any) {
    const { email, itinerary_id, time_to_attend, payment_type, promoCode } =
      req.body;
    const touristService: TouristService = Container.get(TouristService);
  ...
  public async addProductToWishlist(
    ...
    const touristService: TouristService = Container.get(TouristService);
    const wishlist = await touristService.addProductToWishlistService(
      email,
      productObjectID
    );
  ...
  public async removeProductFromWishlist(
    ...
    const productObjectID = new Types.ObjectId(product_id);
    const touristService: TouristService = Container.get(TouristService);
    const wishlist = await touristService.removeProductFromWishlistService(
      email,
      productObjectID
    );
    res.status(wishlist.status).json(wishlist);
  ...
  public async addAddress(req: Request, res: Response): Promise<void> {
    ...
    const touristService: TouristService = Container.get(TouristService);
    const addressResponse = await touristService.addDeliveryAddressService(
      email,
      addressInfo
    );
  ...
  public async removeAddress(req: Request, res: Response): Promise<void> {
    ...
    const touristService: TouristService = Container.get(TouristService);
    const addressResponse = await touristService.removeDeliveryAddressService(
      email,
      address
    );
    res.status(addressResponse.status).json(addressResponse);
  ...
  public async Addpreference(req: any, res: any) {
    const { email } = req.params;
    const preference_id = req.query.preference_id;
    const touristService: TouristService = Container.get(TouristService);
    const preferences = await touristService.AddPreferencesService(
      email,
      preference_id
    );
    res.status(preferences.status).json(preferences);
  }
  public async RemovePreference(req: any, res: any) {
    const { email } = req.params;
    const preference_id = req.query.preference_id;
    const touristService: TouristService = Container.get(TouristService);
    const preferences = await touristService.RemovePreferencesService(
      email,
      preference_id
    );
    res.status(preferences.status).json(preferences);
