...
import { IHistorical_locationDTO, Update_IHistorical_locationDTO } from "@/interfaces/IHistorical_Location";
...
export class Historical_locationController {
  ...
  public async getAllHistorical_locationsController(req: any, res: any) {
    const { role } = req.body;
    const historical_locationService: Historical_locationService = Container.get(Historical_locationService);
    const historical_location = await historical_locationService.getAllHistorical_locationsService(req.query, role);
  ...
  public async createHistorical_locationController(req: any, res: any) {
    ...
    const historical_location = await historical_locationService.createHistorical_locationService(Historical_locationData);
    res.status(historical_location.status).json(historical_location);
  }
  public async getHistorical_locationByIDController(req: any, res: any) {
    const historical_locationService: Historical_locationService = Container.get(Historical_locationService);
    const historical_location = await historical_locationService.getHistorical_locationByIDService(req.query);
    res.status(historical_location.status).json(historical_location);
  }
  public async getHistorical_locationByIDForGovernerController(req: any, res: any) {
    const historical_locationService: Historical_locationService = Container.get(Historical_locationService);
    const historical_location = await historical_locationService.getHistorical_locationByIDForGovernerService(req.query);
    res.status(historical_location.status).json(historical_location);
  }
  //Get Historical_location by Governer_id
  public async getHistorical_locationsByGovernerIDController(req: any, res: any) {
    const Governer_id = req.params.governer_id;
    const historical_locationService: Historical_locationService = Container.get(Historical_locationService);
    const historical_location = await historical_locationService.getHistorical_locationsByGovernerIDService(Governer_id);
    res.status(historical_location.status).json(historical_location);
  }
  ...
  public async updateHistorical_locationController(req: any, res: any) {
    ...
    const historical_location = await historical_locationService.updateHistorical_locationService(historical_location_id, Historical_locationData);
    res.status(historical_location.status).json(historical_location);
  }
  ...
  public async deleteHistorical_locationController(req: any, res: any) {
    ...
    const historical_locationService: Historical_locationService = Container.get(Historical_locationService);
    const historical_location = await historical_locationService.deleteHistorical_locationService(historical_id);
    res.status(historical_location.status).json(historical_location);
  }
  public async getSearchHistorical_location(req: any, res: any) {
    ...
    const { role } = req.body;
    const historical_locationService: Historical_locationService = Container.get(Historical_locationService);
    const historical_locations = await historical_locationService.getSearchHistorical_locationService(name, category, tag, role);
  ...
  public async getUpcomingHistorical_locations(req: any, res: any) {
    const { role } = req.body;
    const historical_locationService: Historical_locationService = Container.get(Historical_locationService);
    const upcomingHistorical_locations = await historical_locationService.getUpcomingHistorical_locationsService(role);
  ...
  public async getFilteredHistorical_locations(req: any, res: any) {
    ...
    const historical_locations = await historical_locationService.getFilteredHistorical_locationsService(filters, role);
  ...
  public async getFilterComponents(req: any, res: any) {
    const { role } = req.body;
    const historical_locationService: Historical_locationService = Container.get(Historical_locationService);
    const filterComponents = await historical_locationService.getFilterComponentsService(role);
