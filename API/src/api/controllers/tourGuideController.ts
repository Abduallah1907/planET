...
import { ITour_GuideUpdateDTO, ITourGuideInput } from "@/interfaces/ITour_guide";
...
export class TourGuideController {
  ...
  public async updateProfile(req: Request, res: Response): Promise<any> {
    ...
    const tourGuideService: TourGuideService = Container.get(TourGuideService);
    const updatedProfile = await tourGuideService.updateProfileService(tourGuideUpdatedData, email);
    res.status(updatedProfile.status).json(updatedProfile);
  }

  public async deleteTourGuideAccountRequest(req: Request, res: Response): Promise<any> {
    const { email } = req.params;
    const tourGuideService: TourGuideService = Container.get(TourGuideService);
    const deletionRequestResponse = await tourGuideService.deleteTourGuideAccountRequest(email);
