...
export default class Historical_locationService {
  ...
  public async getFilteredHistorical_locationsService(
    filters: {
      tags?: string[];
      nation: string;
      job: string;
  ...
  public async getCommentsService(
    ...
    const historical_location = await this.historical_locationsModel
      .findById(historical_location_id)
      .populate("comments");
    if (!historical_location)
      throw new NotFoundError("Historical location not found");
    return new response(
      true,
      historical_location.comments,
      "Comments fetched",
      200
    );
  }
