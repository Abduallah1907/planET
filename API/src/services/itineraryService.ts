...
export default class ItineraryService {
  ...
  public async deleteItineraryService(itinerary_id: Types.ObjectId) {
    ...
    if (tickets.length > 0) throw new BadRequestError("Itinerary booked by some users so cannot delete");
  ...
  public async getSortedItinerariesService(sort: string, direction: string, stakeholder_id: ObjectId, role: string) {
    ...
    let itineraries;
    if (role === UserRoles.Admin) itineraries = await this.itineraryModel.find({}).sort(sortCriteria);
    else itineraries = await this.itineraryModel.find({ active_flag: true, inappropriate_flag: false }).sort(sortCriteria);
    if (itineraries instanceof Error) throw new InternalServerError("Internal server error");
