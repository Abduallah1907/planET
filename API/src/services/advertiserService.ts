...
import { IAdvertiser, IAdvertiserMain, IAdvertiserUpdateDTO } from "@/interfaces/IAdvertiser";
...
export default class AdvertiserService {
  ...
  public createAdvertiserMainDataService = async (advertiserData: IAdvertiserMain) => {
    const IUserInputDTO = {
      email: advertiserData.email,
      name: advertiserData.name,
      username: advertiserData.username,
  ...
  public getAllAdvertisersService = async () => {
    ...
    if (advertisers instanceof Error) throw new InternalServerError("Internal server error");
  ...
  public getAdvertiserByIDService = async (id: string) => {
    const advertiser = await this.advertiserModel.findById(new Types.ObjectId(id));
    if (advertiser instanceof Error) throw new InternalServerError("Internal server error");
    if (advertiser == null) throw new NotFoundError("No Advertiser Found");
    return new response(true, advertiser, "Advertiser found", 200);
  };
  ...
  //Update Advertiser
  public updateAdvertiserService = async (email: string, file: Express.Multer.File, advertiserData: IAdvertiserUpdateDTO) => {
    const { newEmail, name, username, password, phone_number, link_to_website, hotline, about, logo, company_profile } = advertiserData;
    let hashedPassword;
    if (password) {
  ...
  public async deleteAdvertiserAccountRequest(email: string): Promise<any> {
    ...
    if (!advertiserUser || advertiserUser.role !== UserRoles.Advertiser) throw new NotFoundError("Advertiser user account was not found");
