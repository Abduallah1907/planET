...
class TouristService {
  ...
  public static addProductToWishlist = async (
    email: string,
    productId: string
  ) => {
    try {
  ...
  public static removeProductFromWishlist = async (
    email: string,
    productId: string
  ) => {
    try {
      const response = await axiosInstance.delete(
        ...
        {
          data: { product_id: productId }, // Use `data` to send the request body in DELETE
        }
      );
