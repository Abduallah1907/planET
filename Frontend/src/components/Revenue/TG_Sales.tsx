...
const formatDate = (date: string) => Utils.formatDateDay(new Date(date));
...
const TG_Sales: React.FC = () => {
  ...
  const fetchSales = async () => {
    ...
    try {
      if (!TG.email) throw new Error("No email found. Please log in.");
  ...
  const handleFilterChange = (
    field: "typeFilter" | "nameFilter" | "touristCountFilter",
    value: string
  ) => {
  ...
  const handleRequestSort = (property: keyof TG_Sales) => {
    const isAsc = orderBy === property && sortDirection === "asc";
    setSortDirection(isAsc ? "desc" : "asc");
    setOrderBy(property);
  };

  const sortData = (data: TG_Sales[]) => {
  ...
  const filteredSales = sales.filter((sale) => {
    return (
      ...
      (filters.touristCountFilter === "" ||
        sale.tourist_count === parseInt(filters.touristCountFilter))
    );
  });
